...
const logMiddleware = (request, response, next) => {
  ...
  next();
  // Sinon, envoyer une réponse non authentifiée
};

const authenticationMiddleware = (request, response, next) => {
  const apiKey = 'YOUR_API_KEY'; // Remplace avec ta clé API réelle

  const providedKey = request.headers['api-key']; // Assure-toi d'utiliser un header approprié

  if (providedKey && providedKey === apiKey) {
    // Authentification réussie
    next();
  } else {
    createResponse(response).status(401).send('Unauthorized');
  }
};
