...
class Router {
  ...
  logResponse(request, response) {
    ...
    if (response.statusCode < 400) {
      console.log(`${timestamp} - ${method} ${path} ${response.statusCode}`.green);
    } else if (response.statusCode < 500) {
      console.log(`${timestamp} - ${method} ${path} ${response.statusCode}`.yellow);
    } else if (response.statusCode >= 500) {
      console.log(`${timestamp} - ${method} ${path} ${response.statusCode}`.red);
    }
  }
}
